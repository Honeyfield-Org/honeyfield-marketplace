...
Marketplace mit dem `honeyfield-marketing-mcp`-Plugin (Marketing-Ops-MCP über `mcp.ads.honeyfield.at` + DACH-Skills). Skills liegen unter `plugins/<plugin>/skills/<skill>/SKILL.md` (+ optional `references/`, `evals/`) und werden auto-discovered — kein Eintrag in `plugin.json` nötig.

## Pflicht für JEDEN Skill: Projekt-Kontext zuerst
Jeder Skill (Audit wie Creation) **muss** früh — in Schritt 0 / „Vorbereitung" — diesen Absatz enthalten (letzter Satz an die Aufgabe anpassbar):
...
- `python3 -m json.tool <skill>/evals/evals.json` — Evals valides JSON.
