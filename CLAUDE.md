...
- **Skill-Struktur nach Typ.** Gemeinsam für alle: Beleg-Stufen, Schritt 0 (Projekt-Kontext-Absatz + `list_workspaces`/`sources`-Check + Markt-Kalibrierung), DACH-Layer, `evals/`, „Verwandte Skills". Der Aufbau danach hängt vom Typ ab:
  ...
  - **Creation-Skills** (`ad-creative`): Ehrlichkeits-Modell aus der Tool-Reality (vor den Modi — Modi und Operator bauen darauf auf) → **Modi** statt Blocker-Phasen (z.B. neu generieren / aus Performance iterieren) → Output → **Operator** (schreibt nur nach Bestätigung, z.B. Anzeigen als `PAUSED`) → Grenzen.
