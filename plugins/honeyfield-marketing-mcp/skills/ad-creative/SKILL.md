...
Der Moat ist nicht „Claude schreibt Texte”, sondern fünf Dinge, die generisches Claude nicht hat: (1) Angles aus echten Konto-Daten statt erfundener Kategorien, (2) harte Zeichen-Disziplin gegen deutsche Komposita (30/90), (3) DACH-Werberecht (UWG/PAngV/Health) als Leitplanke, (4) Belegpflicht für Claims, (5) ein sicherer Write-Operator (Anlage nur als `PAUSED`, Selbst-Dry-Run, Bestätigung). Dieser Skill ist der Creation-Gegenpart zu `google-ads-audit`: der Audit *findet* schwache/fehlende Creatives, `ad-creative` *füllt* die Lücke.
...
**Projekt-Kontext zuerst.** Liegt für dieses Projekt ein Projekt-Kontext vor — als **Projektwissen** in diesem Claude-Projekt oder als `projekt-kontext.md` im Arbeitsverzeichnis —, nutze ihn (Brand-Tonalität, USPs/Value-Props, Zielgruppe, Ziel-Keywords, belegbare Zahlen für Claims), bevor du fragst, und frage nur nach, was dort fehlt oder für diese Aufgabe spezifisch ist. Beachte gesetzte `compliance`-Flags als harte Leitplanke (z. B. `health`/`medical` → nur EU-zugelassene Health-Claims, keine Wirkversprechen; „keine Superlative” → Spitzenstellungs-Claims blocken). Fehlt der Kontext, biete an, ihn per `projekt-kontext` anzulegen, oder frage knapp: Brand-Ton, USPs, Zielmarkt (DE/AT/CH), Ziel-Kampagne/Ad-Group, belegte Zahlen für Social-Proof.
...
**3. Statistik-Hygiene (bevor „Gewinner/Verlierer” fällt).**
- Mindest-Datengrundlage (Richtwerte): **≥ 1.000 Impressions** pro verglichener Ad, **≥ 10–20 Conversions** für Conversion-Urteile, vergleichbarer Zeitraum (keine Peak-vs-Off-Peak-Vergleiche).
- **Eine Variable pro Zyklus** — nicht gleichzeitig Copy + Bid + Targeting ändern und dann die Copy-Wirkung beurteilen.
- Dünne Datenlage → explizit sagen: „Zu wenig Daten für ein zuverlässiges Urteil (X Impressions) — noch Y Wochen laufen lassen.” Kein Urteil bei < 500 Impressions oder in der Lernphase direkt nach einem Replace.

Details: `references/rsa-mechanik.md` (Abschnitt Statistik-Hygiene).
...
- **Health-Claims / HWG** (`compliance: [health]` / `[medical]`): Positiv-Listen-System — nur EU-zugelassene Angaben. Wirkversprechen („stärkt das Immunsystem”, „Detox”, „heilt”) blocken → neutraler Reframe auf die Nährstoff-Funktion. Bei Arzneimitteln/Medizinprodukten keine Heilversprechen an Laien.
...
- **Sitelinks:** `ads_create_sitelink` / `ads_update_sitelink` (eine Text-Änderung erzeugt ein Ersatz-Asset + Re-Linking → dem Nutzer sagen; nur Sitelink-`final_url` geht echt in-place).
...
## Tools nach Modus
- **Vorbereitung:** `list_workspaces`, `ads_list_campaigns`, `ads_list_ad_groups`
- **Modus A (Fundierung):** `ads_search_terms`, `ads_ai_max_search_terms`, `ads_keyword_performance`
- **Modus B (Fundierung):** `ads_list_ads`, `ads_ad_performance` (+ Befunde aus `google-ads-audit`)
- **Write:** `ads_create_ad_group`, `ads_create_ad` (`status="PAUSED"`), `ads_replace_ad` (`keep_old=true`), `ads_update_ad_status` (Freigabe, bewusst getrennt), `ads_create_sitelink`, `ads_update_sitelink`
