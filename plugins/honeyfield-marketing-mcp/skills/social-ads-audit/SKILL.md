...
Die Operator-Tools unten haben **`validate_only`** — bei `meta_create_campaign`, `meta_update_campaign`, `meta_create_adset`, `meta_update_adset` und `meta_update_ad_status` ein echter API-Dry-Run (Meta prüft den Aufruf, ohne etwas zu ändern); `meta_create_ad` und alle `linkedin_*`-Schreib-Tools prüfen nur lokal (Parameter-Vorschau ohne API-Call). **Kein `validate_only`** bei `meta_upload_ad_image`/`meta_upload_ad_video`, `meta_create_pixel`, `meta_delete_campaign` und `meta_create_custom_conversion` (Meta ignoriert das Flag dort, das Tool bricht bei `validate_only=true` mit Fehler ab) — dort Preview zeigen und bestätigen lassen, ohne Dry-Run. Regel: **erst mit `validate_only=true` ausführen und das Ergebnis als Preview zeigen (was genau, welche Ebene, welche Wirkung, reversibel ja/nein), dann einzeln bestätigen lassen, dann echt schreiben.**
...
- Anzeigen-Inhalte: bei Meta lesbar via `meta_list_ads` (`body` = Anzeigentext, `title`, `call_to_action_type`, `image_url`/`video_id`, `preview_link`, `creative.link_url` = Zielseite, `creative.url_tags`, `creative.lead_gen_form_id`) — das Visual selbst bleibt ein Link, kein Bild-Urteil aus dem Tool; LinkedIn liefert nur die Post-URN — Inhalts-Urteile dort über die Post-URL bzw. den Kunden.
