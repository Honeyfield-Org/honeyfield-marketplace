...
- **Gemessen:** Daten fließen real — Per-Event-Counts (`ga4_list_key_events`, `ga4_report`), `ga4_conversions`, letztes Conversion-Datum / `last_gap_days` (`ads_list_conversion_actions`, `ads_conversion_performance`), gesetzter Link (`ga4_manage_google_ads_links` list), Settings-Get (`ga4_enhanced_measurement` / `ga4_data_retention`). Belastbar für genau das, was sie messen.
- **Nur konfiguriert:** verdrahtet, Firing **unbewiesen** — `gtm_get_tag`, `gtm_list_tags`/`_triggers`/`_variables`, `gtm_get_version`, Conversion-Action-Inventar (`ads_list_conversion_actions`), `ga4_list_custom_dimensions`/`_metrics`. Existiert, beweist aber nicht, dass Daten ankommen.
...
> Grundsatz: **Config ohne Daten = Verdacht, nicht Befund.** Jeder Befund im Report trägt eine dieser drei Stufen. Vollständiges Mapping je Tool in `references/tracking-tool-grenzen.md`.
...
**Markt kalibrieren.** DE/AT/CH bestimmen Data-Retention-Recht und den Consent-Layer (TDDDG §25 / §165 TKG / nDSG). Standard-Analysefenster für die „gemessen”-Checks: die letzten ~30 Tage.
...
- `ga4_manage_google_ads_links` (list) → ist der GA4↔Ads-Link gesetzt? (stärkster „grüner” Check, zugleich Operator-Hebel.)
...
Regel: erst zeigen (was genau, welche Ebene, welche Wirkung), einzeln bestätigen lassen, dann ausführen. **Die Dry-Run-Lage ist uneinheitlich** — vor jeder Schreib-Aktion ausweisen:
- **`validate_only` vorhanden:** `ads_create_conversion_action`, `ads_update_conversion_action`, `ads_upload_conversions`, `ga4_manage_audiences` → erst mit `validate_only` validieren.
- **KEIN Dry-Run (Skill simuliert selbst):** **alle GA4-Writes** (`ga4_create_key_event`, `ga4_create_custom_dimension` / `_metric`, `ga4_enhanced_measurement` / `ga4_data_retention` set) und **alle GTM-Writes** → der Skill zeigt vorher, was sich ändert, und wartet auf Bestätigung. Besonders kritisch bei `ga4_delete_key_event`.
...
**GTM-Flow (4-stufig, einziger live-wirksamer Schritt ist `gtm_publish_version`):**
1. `gtm_list_workspaces` → Workspace-ID ermitteln
2. `gtm_create_trigger` → Trigger anlegen
3. `gtm_create_tag` → Tag anlegen, Trigger-ID in `firing_trigger_ids` setzen
4. `gtm_create_version` → Snapshot erstellen (**nicht live**)
5. **Bestätigung einholen** — Diff der Version zeigen
6. `gtm_publish_version` → **LIVE**, ohne neuen Publish nicht rückgängig
...
- **Kein Setup-from-scratch** (GA4-Property/Stream/Container von Null) — out of scope (ggf. späterer `tracking-setup`).
...
- Phase 1 (Gate): `ads_list_conversion_actions`, `ads_conversion_performance`, `ga4_conversions`, `ga4_list_key_events`
...
- Phase 4 (GTM): `gtm_container_info`, `gtm_get_version`, `gtm_list_tags`, `gtm_list_triggers`, `gtm_list_variables`, `gtm_get_tag`
- Phase 5 (Coverage): `ga4_list_key_events`, `ga4_report`
- Phase 6 (Datenqualität): `ga4_enhanced_measurement`, `ga4_data_retention`, `ga4_list_data_streams`
- Consent-Layer: `gtm_list_tags`, `gtm_get_tag`
- Operator: `ga4_create_key_event`, `ga4_create_custom_dimension`, `ga4_create_custom_metric`, `ga4_enhanced_measurement`, `ga4_data_retention`, `ga4_manage_google_ads_links`, `ads_create_conversion_action`, `ads_update_conversion_action`, `gtm_list_workspaces`, `gtm_create_trigger`, `gtm_create_tag`, `gtm_create_version`, `gtm_publish_version`
