...
| `ads_update_conversion_action` | Bestehende Conversion-Aktion ändern (Name, Status, `default_value`, `primary_for_goal`) — auch `primary_for_goal=False` (Herabstufung auf sekundär) und `default_value=0` werden übernommen; Name/Status danach mit `ads_list_conversion_actions` zurücklesen — `primary_for_goal`/`default_value` zeigt kein Lese-Tool, nur die Antwort bzw. die Google-Ads-Oberfläche | google_ads | W |
