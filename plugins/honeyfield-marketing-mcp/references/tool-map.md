...
| `meta_list_adsets` | Adsets auflisten, optional pro Kampagne (Status, Budget, Optimierungsziel, Targeting, `start_time`/`end_time` = Zeitplan — zum Zurücklesen nach `meta_create_adset`/`meta_update_adset`, nicht-leere `issues`; DSA-Felder immer, `null` = fehlt; `eu_eea_targeting` mit EU-/EWR-Treffern der erfassten Geo-Formen, `[]` = keiner, `UNKNOWN` = nicht auswertbar); `limit` (Default 50) — wird abgeschnitten, kommt `{result, warning}` statt der Liste, dann `campaign_id` einschränken oder `limit` erhöhen | meta_ads | R |
...
| `meta_create_adset` | Neues Adset anlegen (Standard: PAUSED) — Budget (erkennt CBO selbst) + Zielgruppe, Bidding, DSA-Angaben (EU-Pflicht), `advantage_audience`, Zeitplan `start_time`/`end_time` (`end_time` = Traffic-Enddatum, danach pausiert Meta die Auslieferung; je als Datum `2026-09-30` oder ISO-Zeitstempel, ohne Offset gilt die Konto-Zeitzone; mit `meta_list_adsets` zurücklesen); nicht-blockierende DSA-Warnung für alle Geo-Einträge mit Ländercode und `country_groups` `europe`/`eea`/`worldwide` | meta_ads | W |
| `meta_update_adset` | Adset ändern: Name, Status, Budget, Bidding, DSA-Angaben (EU-Pflicht), `advantage_audience`, Zeitplan `start_time`/`end_time` (`end_time` = Traffic-Enddatum; Datum `2026-09-30` oder ISO-Zeitstempel, ohne Offset Konto-Zeitzone; mit `meta_list_adsets` zurücklesen), Targeting (wird gemerged); `bid_amount` OHNE `bid_strategy` liest die bestehende Strategie des Adsets und behält sie bei (COST_CAP bleibt COST_CAP; nur LOWEST_COST_WITHOUT_CAP wechselt auf LOWEST_COST_WITH_BID_CAP), die Antwort nennt in `bid_strategy`, was gesendet wurde (bei CBO-Adsets ohne eigene Strategie fehlt das Feld) — ist die Strategie nicht lesbar, bricht das Tool ohne Änderung ab; gleiche DSA-Coverage, bei `status=ACTIVE` zusätzlicher Best-effort-Advisory-Check (Ausfall → `dsa_check: "unavailable"` + Warnung) | meta_ads | W |
