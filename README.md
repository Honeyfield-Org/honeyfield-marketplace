...
| `social-ads-audit` | Social-Ads-Audit (Meta/LinkedIn) — Pixel-/Signal-Gesundheit, Budget-Verteilung, Objective-/Struktur-Check, Creative-Fatigue, Audiences; zieht echte Konto-Daten (+ GA4-Cross-Check) und setzt Behebbares nach Dry-Run (`validate_only`) + Bestätigung um: pausieren, Budgets anpassen, neue Anzeigen als PAUSED/DRAFT. |
...
| `content-strategie` | Content-Strategie — findet und priorisiert Themen aus echten Suchdaten (GSC + DataForSEO) und liefert Content-Briefs (Query-Set, Gliederung, W-Fragen, interne Verlinkung, Belegpflicht-Hinweise). Read-only: schreibt keine Artikel und published nichts — die Umsetzung liegt beim Kunden. |
